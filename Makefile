...
NETWORK_ARGS := --rpc-url $(ANVIL_RPC_URL) --private-key $(PRIVATE_KEY_ANVIL) --broadcast
...
NETWORK_ARGS1 := --rpc-url $(SEPOLIA_RPC_URL) --private-key $(PRIVATE_KEY_SEPOLIA) --broadcast --verify --etherscan-api-key $(ETHERSCAN_API_KEY) -vvvv

deployMyTokenToAnvil:
	@forge script script/DeployMyToken.s.sol:DeployMyToken $(NETWORK_ARGS)

deployMyTokenToSepolia:
	@forge script script/DeployMyToken.s.sol:DeployMyToken $(NETWORK_ARGS1)


deployTokensCrowdSaleToAnvil:
	@forge script script/DeployTokensCrowdSale.s.sol:DeployTokensCrowdSale $(NETWORK_ARGS)

deployTokensCrowdSaleToSepolia:
	@forge script script/DeployTokensCrowdSale.s.sol:DeployTokensCrowdSale $(NETWORK_ARGS1)
